...
FROM FLIGHTS AS F1,
    (SELECT F2.origin_city AS city, MAX(F2.actual_time) AS time
    FROM FLIGHTS AS F2
    GROUP BY F2.origin_city) AS F3
WHERE F1.origin_city = F3.city
AND F1.actual_time = F3.time;

--334 rows, runtime: 14s 384ms
